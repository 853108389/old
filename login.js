...
import DeviceStorage from './Component/common/deviceStorage';
...
export default class Login extends Component {
    ...
    render() {
        return (
            <View style={styles.container}>
                ...
                <TouchableOpacity style={{ position: "absolute", bottom: 10, left: 10 }} onPress={() => { this._pushRegist() }}>
                    <View >
                        <Text style={{ color: "blue", fontSize: 18 }}>新用户注册</Text>
                    </View>
                </TouchableOpacity>
            </View>
        );
    ...
    _renderStuNoLogin() {
        let url = config.yitai.base + config.yitai.login;
    ...
    //存储用户信息
    _afterLogin(user) {
        DeviceStorage.save("user", user).then(() => {
            this.setState({
                user: user
            });
            // ToastAndroid.show('更新成功', ToastAndroid.SHORT);
        }).then(() => {
            this._pushHome(user.userType);
        }).catch((err) => {
            alert(err);
        });
    }
