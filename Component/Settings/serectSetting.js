...
import { fetchConfirmPw, asyncGetUser, fetchEditPw } from '../common/fetch'
...
export default class SerectSetting extends Component {
    ...
    _fetchEditPw() {
        ...
        fetchEditPw(userId, this.state.content1, this.state.content2).then((result) => {
            if (result == 1) {
    ...
    _fetchConfirmPw() {
        ...
        // data: result.data
        let userId = this.state.userId;
        fetchConfirmPw(userId, this.state.content1).then((result) => {
            if (result == 1) {
                this.setState({
                    isOK1: true,
                }, () => {
